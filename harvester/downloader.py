...
全自动按当天日期生成专属的独立日志文件，彻底杜绝单文件体积过大隐患
"""
...
# 动态组装并安全创建中央 logs 文件夹
log_dir = os.path.abspath(config.LOG_DIR)
os.makedirs(log_dir, exist_ok=True)
current_date_str = datetime.now().strftime("%Y-%m-%d")
log_file_path = os.path.join(log_dir, f"harvest_{current_date_str}.log")
...
class BBCResourceDownloader:
    ...
    def __init__(self):
        ...
        logger.info(f"多轨多站资产收割底座就绪 -> 本轮日志按天定位至: {log_file_path}")
    ...
    def _load_log(self, log_path):
        if os.path.exists(log_path):
            try:
                ...
                logger.error(f"读取去重账本遭遇意外硬盘故障: {e} -> {log_path}")
        return set()
    ...
    def _request_with_retry(self, url, max_retries=3, initial_delay=2):
        ...
        for attempt in range(max_retries):
            try:
                logger.debug(f"发起网络请求 (尝试第 {attempt+1}/{max_retries} 次) -> URL: {url}")
    ...
    def _save_article(self, feed_key, title, link, pub_date_str, url_log_path):
        ...
        if len(full_text) > length_threshold:
            ...
            self.seen_urls.add(link)
            self._write_log(url_log_path, link)
            logger.info(f"纯净 Markdown 单文件成功落盘: {filename} [专栏: {feed_key}]")
            return "success"
        else:
            logger.warning(f"图文线阈值拦截 -> 清洗后字数仅为 {len(full_text)} 字符，未达精读线阈值({length_threshold}): {link}")
            return "failed"
    ...
    def process_single_snapshot(self, feed_key, snapshot_url, url_log_path):
        ...
        try:
            ...
            logger.error(f"解析历史快照 XML 核心树发生崩溃: {e} -> {snapshot_url}")
            return False
    ...
    def sync_latest(self, target_keys=None):
        ...
        for name in target_keys:
            ...
            try:
                ...
                logger.error(f"多站实时增量同步出现异常 [故障频道: {name}]: {e}")
    ...
    def download_history(self, target_keys=None, start_year=2020, end_year=None):
        ...
        for name in target_keys:
            ...
            try:
                ...
                logger.error(f"多站历史归档清洗发生异常崩溃: {e} -> 故障版块: {name}")
